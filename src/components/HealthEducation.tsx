...
//   // state populated from backend (if available)
//   const [educationItems, setEducationItems] = useState<EducationItem[] | null>(null);
...
//   useEffect(() => {
//     // Try to fetch education items from backend on mount.
//     // fetchEducationItems respects USE_MOCK in services/api.ts so it will return mock
//     // data if mock mode is enabled.
//     const load = async () => {
//       try {
//         setLoading(true);
//         setFetchError(null);
//         const items = await fetchEducationItems();
//         // Map backend shape to UI shape if necessary: ensure id, title, summary/description, category, duration, content
//         const mapped: EducationItem[] = (items || []).map((it: any, idx: number) => ({
//           id: it.id ?? `be-${idx}`,
...
//         } else {
//           // leave null to fall back to translations topics
//           setEducationItems([]);
//         }
//       } catch (err: any) {
//         // if backend fails, keep fallback topics from translations
//         setFetchError(err?.message ?? 'Failed to load education content');
//         setEducationItems([]); // signal "no backend items" fallback
//       } finally {
//         setLoading(false);
//       }
//     };
...
//   // choose items: prefer backend items (educationItems) if present and non-empty,
//   // otherwise fallback to language-specific topics in translations
//   const sourceTopics = (educationItems && educationItems.length > 0)
//     ? // convert EducationItem[] to the topic shape used by the page
//       (educationItems.map(it => ({
//         id: it.id,
//         category: (it.category || 'General').toLowerCase(),
//         title: it.title,
//         description: it.summary || '',
//         duration: (it as any).duration || '—',
//         content: (it as any).content || it.summary || ''
//       })))
//     : t.topics;

//   const filteredTopics = selectedCategory
//     ? sourceTopics.filter(topic => topic.category === selectedCategory)
//     : sourceTopics;

//   if (selectedTopic) {
//     const topic = sourceTopics.find(t2 => t2.id === selectedTopic);
//     if (!topic) return null;
...
//       {loading && (
//         <Card>
//           <CardContent className="p-4 text-gray-600">Loading education content...</CardContent>
//         </Card>
//       )}
//       {fetchError && (
//         <Card>
//           <CardContent className="p-4 text-red-600">Could not load server content — showing local topics.</CardContent>
//         </Card>
//       )}
...
//           const Icon = categoryIcons[topic.category as keyof typeof categoryIcons] ?? Shield;
//           const colorClass = categoryColors[topic.category as keyof typeof categoryColors] ?? 'bg-gray-100 text-gray-700';
