...
const translations = {
  ...
  te: {
    ...
    viewDetails: 'వివরాలు చూడండి',
    searchPlaceholder: 'టీకాలను వెతకండి...',
    nextDue: 'తదుপরి రావాল్సিన తేदీ',
    vaccineCenter: 'టీకా కেంద్రాన్ని కనుগొనండి'
  },
  bn: {
  ...
  en: {
    infant: [
      ...
      },
      {
        ...
        age: 'At birth, 6 weeks, 6 months',
        status: 'due',
        nextDue: '2024-01-15'
      },
      {
        name: 'DPT',
        description: 'Protects against Diphtheria, Pertussis, Tetanus',
        age: '6, 10, 14 weeks',
        status: 'upcoming',
        nextDue: '2024-02-01'
      },
    ...
    senior: [
      ...
      {
        name: 'Shingles',
        description: 'Protects against shingles (zoster)',
        age: '60+ years',
        status: 'upcoming',
        nextDue: '2024-04-01'
      }
...
export function VaccinationSchedule({ language }: VaccinationScheduleProps) {
  ...
  const vaccines = vaccineData.en; // Using English data for all languages for now

  const filteredVaccines = vaccines[selectedAgeGroup as keyof typeof vaccines]?.filter(vaccine =>
    vaccine.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    vaccine.description.toLowerCase().includes(searchTerm.toLowerCase())
  ) || [];

  const getDueSoonCount = () => {
    return Object.values(vaccines).flat().filter(v => v.status === 'due').length;
  };
    ...
    <div className="space-y-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-lg">Jan 5, 2024</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4">
        {filteredVaccines.map((vaccine, index) => {
          const StatusIcon = statusIcons[vaccine.status as keyof typeof statusIcons];
          const statusClass = statusColors[vaccine.status as keyof typeof statusColors];
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <h3 className="text-lg">{vaccine.name}</h3>
                      <Badge className={statusClass}>
                        <StatusIcon className="w-3 h-3 mr-1" />
                        {t.status[vaccine.status as keyof typeof t.status]}
                      </Badge>
                    </div>
                    
                    <p className="text-gray-600 text-sm mb-2">{vaccine.description}</p>
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-500">
                      ...
                      {vaccine.nextDue && (
                        <div className="flex items-center space-x-1">
                          ...
                          <span>Due: {new Date(vaccine.nextDue).toLocaleDateString()}</span>
                        </div>
                      )}
                    </div>
                  </div>
...
//   const getDueSoonCount = () => {
//     return Object.values(vaccines).flat().filter(v => normalizeStatus(v.status) === 'due' || normalizeStatus(v.status) === 'overdue').length;
//   };
...
//                     </Button>
//                     {uiStatus !== 'completed' && (
//                       <Button size="sm" onClick={() => handleSetReminder(vaccine)}>
//                         {t.addReminder}
//                       </Button>
//                     )}
//                   </div>
